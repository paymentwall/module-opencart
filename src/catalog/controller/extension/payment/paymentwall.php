...
class ControllerExtensionPaymentPaymentwall extends Controller
{
    public function index()
    {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/paymentwall.tpl')) {
            return $this->load->view($this->config->get('config_template') . '/template/extension/payment/paymentwall.tpl', $data);
        } else {
            return $this->load->view('extension/payment/paymentwall.tpl', $data);
        }
    }
    ...
    public function widget()
    {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/paymentwall_widget.tpl')) {
            $template = $this->config->get('config_template') . '/template/extension/payment/paymentwall_widget.tpl';
        } else { 
            $template = 'extension/payment/paymentwall_widget.tpl';
        }
    ...
    protected function prepareViewData($orderInfo, $customer)
    {
        ...
        $data['iframe'] = $this->model_extension_payment_paymentwall->generateWidget($orderInfo, $customer, $this->config->get('paymentwall_success_url'));
